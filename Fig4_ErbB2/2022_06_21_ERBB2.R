...
setwd("/Users/payamfarahani/Documents/Python_R/RTK BIOSENSORS/ERBB2/2022_06_21_ERBB2");
...
# enter parameters
frame_pre <- 6; # final frame before ligand addition
time_ligand <- 60; # ligand added after this time point
...
for (i in seq_along(ZtSH2_names)) {
  ...
  matrix_melt <- mutate(matrix_melt, 
                        condition = str_replace(str_extract(ZtSH2_names[[i]],"[:alpha:]*_"),"_",""), 
...
p1 <- ggplot() +
  ...
  geom_ribbon(data = dat, aes(x = time,
                                       ...
                                       fill = condition),
              alpha = 0.2) +
  labs(x = "time (min)", y = "ErbB2 activity") +
